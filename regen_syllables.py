...
# 3. Triplas (C1 + C2 + R + V) - USING FULL LETTERS AND 'R' SEPARATOR
...
# 4. Encontros Vocálicos Independentes (V + R + V) -> Wait! 
# Previously this was V + RR + V. If we use V + R + V it collides with Invertidas!
# So Encontros Vocálicos MUST keep RR!
for v1 in vowels:
    for v2 in vowels:
        if v1 == v2: continue
        add_seq(letters[v1] + 'RR' + letters[v2], v1 + v2)
...
# 7. Triplas + Suffix_Consonant (ex: bram, pran)
for mod, cons_list in valid_triples.items():
    for c in cons_list:
        for v in vowels:
            if mod == 'u' and v == 'u': continue
...
for s in list(generated.keys()):
    if s in used:
        ...
        val = generated[s]
        del generated[s]
        add_seq(s + 'Q', val) # resolve by appending Q
