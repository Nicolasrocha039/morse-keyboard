...
for c in consonants:
    for v in vowels:
        for term, suf in suffixes_cons.items():
            # suf is already R + letter, so we just append it
...
for c in consonants:
    for v in vowels:
        for v2, suf in suffixes_vowels.items():
            if v == v2: continue
            # suf here is just the letter, we need to prepend R
...
for mod, cons_list in valid_triples.items():
    for c in cons_list:
        for v in vowels:
            if mod == 'u' and v == 'u': continue
            for term, suf in suffixes_cons.items():
                add_seq(letters[c] + 'R' + letters[mod] + 'R' + letters[v] + suf, c + mod + v + term)
